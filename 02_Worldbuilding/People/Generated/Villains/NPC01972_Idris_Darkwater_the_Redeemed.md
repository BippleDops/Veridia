# Idris Darkwater the Redeemed

*Tiefling Assassin (Villains), Age 74, Male*

**ID**: NPC01972

## Quick Reference
- **Location**: City gates at shift change
- **Occupation**: Black market dealer
- **Disposition**: Never backs down from any challenge

## Appearance
Very short and muscular, with Long and flowing silver hair and Gray eyes. Olive skin. Gold tooth that glints when smiling.

**Clothing**: Sailor's outfit with salt stains
**Accessories**: Decorated dagger

## Personality
- **Traits**: Never backs down from any challenge; Always speaks in metaphors and riddles; Believes in every conspiracy theory
- **Ideal**: Charity: I always help those in need, no matter the cost
- **Bond**: My weapon has been passed down for generations
- **Flaw**: I have a terrible and obvious gambling problem
- **Quirks**: Always barefoot, Counts everything obsessively
- **Fear**: The undead
- **Dream**: Finding true love

## Background
Driven mad by knowledge humanity wasn't meant to know A chance encounter last winter changed their perspective on everything.

## Current Situation
- **Occupation**: Black market dealer
- **Resources**: Blackmail material, Hidden bases, Corrupt officials
- **Problems**: Massive debt to dangerous people

### Goals
- **Short Term**: Make this month's payment
- **Long Term**: Discover immortality


## Relationships
### Delara Keenblade the Swift (Family - Spouse)
- **Status**: Blood oath bound
- **History**: Bonded over shared tragedy


### Delara Zephyrblade the Wise (Criminal - Informant)
- **Status**: Complicated history
- **History**: Bitter rivals who learned respect
- **Secret**: Secretly in love

### Pyria Dawnstrider the Guardian (Family - Cousin)
- **Status**: Strained but maintaining
- **History**: Met during a heist gone wrong
- **Secret**: Secretly in love

### Erasmus Crystalbrook the Cunning (Professional - Employee)
- **Status**: Recently reconciled
- **History**: Accidentally bound by magic
- **Secret**: One is a shapeshifter

## Secrets
1. Knows how the world will end

## Statistics
- **Level**: 8
- **HP**: 66
- **AC**: 13
- **Speed**: 30 ft.
- **Challenge Rating**: 4

### Abilities
| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 16 | 14 | 11 | 12 | 14 | 8 |

### Saving Throws
CON, CHA

### Skills
Stealth, Sleight of Hand, Nature, Deception

### Attacks
- Poisoned blade: +4 to hit, 1d6+3 slashing
- Hidden dagger: +7 to hit, 1d4+3 slashing

### Special Abilities
- Dark Blessing: Resistance to one damage type
- Summon Minions: Call 1d4 lesser creatures
- Frightening Presence: Enemies make WIS save or frightened
- Spellcasting: 5 spell slots, knows Scorching Ray, Misty Step, Shield, Fireball, Charm Person, Fly, Lightning Bolt

## Possessions
- 489 gold pieces
- Ritual components
- Ritual components

## Dialogue
### Greetings
- "You're either very brave or very foolish."
- "I don't believe we've been introduced..."
- "This is a dangerous place to wander."

### Catchphrases
- "Everyone has a price."
- "If you say so."

### Knowledge
- Blackmail material
- Guard patrol schedules
- Hidden passages
- Rival gang territories

## Plot Hook
Blackmails party into service

## DM Notes
- Adjust stats based on party level
- Secrets can drive major plot points
- Relationships create web of connections
- Use quirks for memorable roleplay

---
*Generated: 2025-08-15T03:51:03.330Z*
