---
aliases: ["Champion's Redemption", "champion's redemption", "Redemption Champion's"]
tags: [generated]
id: QUEST0024
created: 2025-08-15T04:09:51.377Z
generator: mega_generator_10k
---
# Champion's Redemption

*Fetch Quest for levels 2-6*

**ID**: QUEST0024

## Quest Giver
**Yorick Goldleaf** - Mysterious stranger
- **Location**: Temple district
- **Motivation**: Desires power and influence

## Objectives
### Primary
Retrieve the ancient artifact from the sunken temple

### Secondary
- Gather intelligence on enemy plans
- Rescue any prisoners found

### Hidden Objective
*DM Only: Employer has ulterior motives*

## Background
Years ago, a series of events set this quest in motion. undefined. Years ago, they made a deal they now regret, which changed their life forever. Now, circumstances demand immediate action.

## Key Locations
- **Hillford** (City): Key location for quest progression
- **Valeford** (Dungeon): Key location for quest progression
- **Marshford** (Wilderness): Key location for quest progression

## Opposition
### Enemies
- **Cultists** led by Yorick Brightblade the Brave
  - Strength: 20 combatants
  - Tactics: Aggressive but cunning

### Obstacles
- Time-sensitive element
- Moral dilemma

## Complications
- Resource shortage
- Resource shortage

## Plot Twist
*Reveal at appropriate moment:*
- Enemies become necessary allies

## Moral Choices
### Enemy offers valuable information for mercy
**Options**:
1. Show mercy and gain intel - Future ally but justice denied
2. Execute for crimes committed - Justice served but opportunity lost

## Time Limit
⏰ **24 hours before execution**

## Rewards
### On Success
- **Gold**: 1,941 gp
- **Experience**: 2,000 XP

- **Other**: Ancient knowledge

## Consequences
- **Success**: Ancient evil remains sealed
- **Failure**: Darkness consumes the land

## DM Notes
- Adjust difficulty based on party composition
- The hidden objective adds complexity for experienced players
- Moral choices should have lasting campaign impacts
- Consider how this quest ties into larger campaign arcs

---
*Generated: 2025-08-15T03:21:04.087Z*


## Timeline
- Key events
- Deadlines


## NPCs Involved
- Quest giver
- Antagonists


## Locations
- Starting point
- Key locations


## Alternative Solutions
- Non-combat options
- Creative approaches
