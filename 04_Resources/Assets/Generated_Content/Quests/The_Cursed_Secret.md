---
tags: [quest, adventure]
type: quest
---

# The Cursed Secret

*A fetch quest for 4-6 characters of levels 3-7*

## Overview
**Quest Giver**: Fiora Frostbeard (Gnome Rogue)  
**Location**: Guarding a merchant caravan  
**Type**: Fetch Quest  
**Objective**: Retrieve the stolen artifact from the bandit hideout

## Background
Fiora Frostbeard searches for a cure to a mysterious affliction. Suspicious of strangers, they order and law. However, can't resist a pretty face, which may complicate matters.

## Quest Stages

### Stage 1: The Hook
Meet with Fiora Frostbeard and learn about the problem

**Objectives**:
- Speak with the quest giver
- Gather initial information
- Accept the quest

**Key Locations**: Guarding a merchant caravan

### Stage 2: Investigation
Gather information and prepare for the main objective

**Objectives**:
- Interview key witnesses or informants
- Research background information
- Acquire necessary equipment or allies

**Key Locations**: Local tavern, City archives, Market district

### Stage 3: Challenge 1
Navigate obstacles to reach the target item

**Objectives**:
- Bypass security
- Solve the puzzle
- Defeat guardians

**Key Locations**: Working at the docks

### Stage 4: Challenge 2
Navigate obstacles to reach the target item

**Objectives**:
- Bypass security
- Solve the puzzle
- Defeat guardians

**Key Locations**: Hiding in the sewers beneath the city

### Stage 5: Resolution
Complete the quest and claim your reward

**Objectives**:
- Return to quest giver
- Report success
- Collect reward

**Key Locations**: Guarding a merchant caravan

## Complications
These complications may arise during the quest:
- An unexpected ally becomes an enemy

## Moral Choices

### Midway through the quest
**Decision**: Choose between a safe but longer route or a dangerous shortcut

**Consequences**:
- **Option A**: Arrive late but intact, missing an opportunity
- **Option B**: Risk combat but potentially gain valuable information

### At the climax
**Decision**: Decide whether to show mercy or deliver justice

**Consequences**:
- **Option A**: Gain an ally but risk future problems
- **Option B**: Solve the immediate problem but make enemies

## Rewards
Upon successful completion:
- 508 gold pieces
- 1141 XP per character
- Cloak of Protection
- Favor with local nobility

## Failure Consequences
If the party fails:
- Fiora Frostbeard loses faith in the party
- The situation worsens (specific consequences depend on quest type)
- Rival parties may succeed where the party failed
- Future opportunities may be lost

## DM Notes
- Adjust combat encounters based on party strength
- The quest giver's flaw (can't resist a pretty face) can create additional roleplay opportunities
- Consider adding environmental challenges based on the locations
- The moral choices should have lasting consequences in your campaign

## Scaling Options
- **Lower Level (1-3)**: Reduce combat difficulty, simplify puzzles, increase guidance
- **Higher Level (8-10)**: Add additional complications, increase combat CR, add time pressure
- **Larger Party**: Add more enemies, increase skill check DCs
- **Smaller Party**: Provide NPC assistance, reduce enemy numbers

---
*Generated: 2025-08-15T03:07:45.516Z*


## Timeline
- Key events
- Deadlines


## NPCs Involved
- Quest giver
- Antagonists


## Locations
- Starting point
- Key locations


## Alternative Solutions
- Non-combat options
- Creative approaches
