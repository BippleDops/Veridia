---

title: Campaign Overview
type: Campaign
tags: 
- adventure
- session-notes
- campaign/crystal-plague
- quest
- cure-quest
- world/both
- status/active
- active
- combat
- corruption
- medical-mystery
- crystal-enhanced
created: '2025-08-13'
modified: '2025-08-14'
status: active
level: 1-5
updated: '2025-08-13T17:20:00.000000'
world: Both
evaluated: '2025-08-14'
improvements: 20
---

# Crystal Plague Cure: Medical Mystery Campaign

*A Race Against Time to Save Queen Seraphina and Stop the Crystal Corruption*

> [!warning] Campaign Themes
> - **Medical Mystery**: Investigating a magical disease
> - **Corruption Spread**: Physical and moral decay
> - **Royal Intrigue**: Queen Seraphina's hidden affliction
> - **Scientific Ethics**: How far to go for a cure
> - **Economic Warfare**: Crystal trade implications

## 🔧 Deep Evaluation Improvements

*20 targeted improvements identified*

### Combat Improvements

- Add encounter scaling table for parties of 3-6 players, levels 1-20

### Rewards Improvements

- Create treasure parcels for each major encounter

### Structure Improvements

- Add three-act structure with rising action

### Choices Improvements

- Add meaningful decision points with consequences

### Roleplay Improvements

- Include ethical challenges without clear answers

### Worldbuilding Improvements

- Connect to three other adventures in the vault

### Spotlight Improvements

- Include spotlight moment for each character class

### Level_Range Improvements

- Provide scaling for different level ranges

### Variants Improvements

- Add horror, intrigue, and comedy tone variants

### Soundtrack Improvements

- Suggest music/ambiance for each scene

### Backup Improvements

- Add 'What if players skip this?' contingency

### Session_Zero Improvements

- Add session zero integration questions

### Accessibility Improvements

- Include content warnings and safety tools

## Campaign Overview

The Crystal Plague has begun spreading through both Aquabyssos and Aethermoor, transforming living beings into crystalline statues while granting them temporary magical power. Queen Seraphina herself is secretly infected, making the cure a matter of royal urgency. The party must research the plague's origin, track its spread, develop a cure, and navigate the political implications of a disease that some view as a blessing.

### Disease Progression

**Stage 1: Crystal Touch (Days 1-7)**
- Small crystal patches on skin
- +1 to AC naturally
- Advantage on saves vs transmutation
- No negative effects yet

**Stage 2: Crystal Growth (Days 8-30)**
- 25% body coverage
- +2 AC, resistance to physical damage
- Disadvantage on Dexterity checks
- Obsession with wealth begins

**Stage 3: Crystal Merger (Days 31-60)**
- 50% body coverage
- +3 AC, immunity to critical hits
- Speed reduced by 10 feet
- Alignment shifts toward lawful evil

**Stage 4: Crystal Domice (Days 61-90)**
- 75% body coverage
- +4 AC, resistance to magic
- Speed reduced by 20 feet
- Original personality suppressed

**Stage 5: Complete Crystallization (Day 91+)**
- Becomes crystal statue
- Consciousness trapped inside
- Can be shattered for valuable crystals
- Some retain telepathic abilities

### The Infected

**Queen Seraphina Lumengarde**
- Stage 2, hiding with illusion magic
- Desperate for cure but maintaining appearances
- Will provide resources but demands secrecy
- Internal struggle against crystal influence

**Lord Aurelius Goldwave**
- Stage 3, embracing the power
- Sees plague as economic opportunity
- Controls crystal trade routes
- May oppose cure efforts

**Dr. Coral Mindbridge**
- Stage 1, lead researcher
- Racing to cure herself
- Has preliminary research
- Needs rare components

### The Researchers

**Alchemist Marina Solvent**
- Immune due to previous exposure
- Knows partial cure formula
- Requires dangerous ingredients
- Morally flexible methods

**High Priest Tidal Blessing**
- Believes plague is divine punishment
- Opposes certain cure methods
- Has ancient texts about similar plagues
- Controls religious healing resources

**Professor Crystalline Theory**
- Studies plague for academic purposes
- Doesn't care about victims
- Has crucial information
- Will trade knowledge for samples

### The Opposition

**The Crystal Cult**
- Worship the plague as transcendence
- Actively spread infection
- Sabotage cure efforts
- Led by fully crystallized prophet

**Silverscale Consortium**
- Profit from crystal harvesting
- Oppose cure development
- Control key resources
- Economic warfare tactics

### Session 1: The Crystal Discovery

*First cases appear in the capital*

**Opening**: Public festival interrupted by crystallizing citizen
**Investigation**: Track patient zero, discover spread pattern
**Revelation**: Queen Seraphina secretly infected
**Combat**: Crystal cult initiates attack researchers
**Cliffhanger**: Multiple infection sites discovered

### Session 2: The Research Race

*Gathering scientific resources and knowledge*

**Locations**: 
- Royal Medical Academy
- Ancient Library of Plagues
- Alchemist Underground Market

**Challenges**:
- Competing research teams
- Sabotage attempts
- Resource shortage
- Time pressure

### Session 3: The First Cure Attempt

*Testing experimental treatments*

**Medical Trials**:
- Volunteer test subjects
- Unexpected side effects
- Partial success
- Moral dilemmas

**Complication**: Cure makes some patients explode into crystal shards

### Session 4: The Plague Source

*Journey to the origin point*

**Location**: Abandoned Silverscale Mine

**Discoveries**:
- Patient Zero's remains
- Connection to Crystal Shard
- Intentional or accidental release?
- Corporate cover-up evidence

### Session 5: The Living Cure

*A person immune to the plague holds the key*

**The Immune**: Young child with natural immunity

**Moral Dilemma**:
- Extract cure harmfully?
- Protect child from factions?
- Use willing volunteer?
- Find alternative method?

### Session 6: Political Complications

*Cure becomes political weapon*

**Faction Conflicts**:
- Who gets cured first?
- Economic implications
- Military advantages
- Diplomatic leverage

### Session 7: The Mass Production

*Manufacturing and distributing cure*

**Challenges**:
- Secure production facility
- Protect supply chains
- Prevent contamination
- Equal distribution

### Session 8: The Crystal Prophet

*Final confrontation with plague's ultimate form*

**Boss**: Fully crystallized being with plague control
**Choice**: Cure all or preserve some for study?
**Consequence**: Economic and social restructuring

### Research Points System

Players need 20 Research Points to develop the cure.
```meta-bind
INPUT[progressBar(minValue(0), maxValue(20)):research-points]
LABEL[Research Points Accumulated]
```
### Research Sources

**Medical Examination** (1-2 points)
- Study infected patients
- Autopsy crystallized victims
- Track progression patterns
- Analyze transmission methods

**Laboratory Work** (2-3 points)
- Test cure components
- Synthesize compounds
- Magical analysis
- Crystal structure study

**Field Work** (1-2 points)
- Track infection spread
- Identify patient zero
- Environmental sampling
- Population studies

**Ancient Knowledge** (3-4 points)
- Historical plague records
- Religious texts
- Arcane formulas
- Lost civilization cures

### Research Complications `dice: `dice: `dice: 1d12```

| d12 | Complication |
|-----|--------------|
| 1-2 | Lab contamination |
| 3-4 | Sabotage attempt |
| 5-6 | Resource shortage |
| 7-8 | Test subject dies |
| 9-10 | False positive result |
| 11-12 | Breakthrough discovery |

### City Plague Encounters `dice: `dice: `dice: 1d20```

| d20 | Encounter |
|-----|-----------|
| 1-2 | Crystallizing citizen needs help |
| 3-4 | Crystal cult recruitment |
| 5-6 | Panicked mob |
| 7-8 | Quarantine checkpoint |
| 9-10 | Black market cure seller |
| 11-12 | Infected noble in denial |
| 13-14 | Overworked healers |
| 15-16 | Crystal harvester |
| 17-18 | Government cover-up |
| 19-20 | Immune individual discovered |

### Research Facility Events `dice: `dice: `dice: 1d10```

| d10 | Event |
|-----|-------|
| 1-2 | Equipment failure |
| 3-4 | Rival researcher |
| 5-6 | Breakthrough moment |
| 7-8 | Funding crisis |
| 9-10 | Ethical review board |

### The Volunteer Dilemma

Desperate infected volunteer for dangerous experiments:
- **Allow**: Fast research but possible death
- **Refuse**: Slower progress but ethical
- **Compromise**: Limited testing with safeguards

### The Triage Choice

Limited cure supplies available:
- **Save the Queen**: Political stability
- **Save the Children**: Future generation
- **Save the Workers**: Economic function
- **Equal Distribution**: Fair but insufficient

### The Crystal Harvest

Crystallized victims contain valuable materials:
- **Harvest**: Fund research but desecrate dead
- **Preserve**: Respect dead but lack resources
- **Selective**: Volunteer donors only

### The Immune Child

Child has natural immunity in their blood:
- **Extract Aggressively**: Fast cure but harm child
- **Protect Completely**: No cure but child safe
- **Negotiate**: Limited extraction with consent

### Dataview Tracking
```dataview
TABLE 
  infection-stage as "Stage",
  days-infected as "Days",
  treatment-status as "Treatment",
  location as "Location"
FROM "Crystal_Plague"
WHERE contains(tags, "infected-npc")
SORT infection-stage DESC
### Meta-bind Controls
INPUT[slider(minValue(0), maxValue(100)):plague-spread]
LABEL[Regional Infection Rate %]
INPUT[slider(minValue(0), maxValue(5)):queen-infection-stage]
LABEL[Queen Seraphina's Infection Stage]
### Initiative Tracker: Crystal Cult Ambush
```initiative-tracker
- name: Crystal Prophet
  hp: 120
  ac: 18
  initiative: 20
  tags: ["boss", "crystallized"]
- name: Crystal Cultist (4)
  hp: 45
  ac: 15
  initiative: 14
  tags: ["minion", "infected"]
- name: Crystal Elemental
  hp: 90
  ac: 17
  initiative: 16
  tags: ["summoned", "magical"]
### Required Cure Components

1. **Pure Water**: From uncontaminated source
2. **Memory Moss**: Grows in ancient ruins
3. **Solar Crystals**: Charged with sunlight
4. **Void Essence**: From dimensional rifts
5. **Life Root**: From eldest trees
6. **Unity Stone**: From merged consciousness
7. **Storm Glass**: From perpetual tempests

### Component Gathering Missions

Each component requires a mini-adventure:
- Combat challenges
- Environmental hazards
- Social negotiations
- Puzzle solving
- Moral choices

## Connected Resources

- [[Crystal Plague Infection Tracking
- [Medical Research Rules]]
- [[Cure Component Locations
- [Infected NPC Database]]
- [[Crystal Cult Organization
- [Queen Seraphina's Progression]]

---

*"In crystal's embrace, we find perfection's curse"* - Dr. Coral Mindbridge's journal

## Synopsis

This adventure involves the party in a series of challenges and discoveries. The stakes are significant, and the outcomes will shape future events.

**Level Range**: 1-5 (adjustable)
**Expected Duration**: 2-4 sessions
**Themes**: Mystery, Combat, Exploration

## Background

The events leading to this adventure have been building for some time. Forces are in motion that the party must confront or redirect.

## Locations

1. **Starting Point**: Where the adventure begins
2. **Investigation Sites**: Places to gather information
3. **Challenge Areas**: Combat or puzzle locations
4. **Climax Location**: Where everything comes together

### Primary Hook
The main reason parties get involved

## DM Notes

*Private notes for campaign integration:*
- Can be adapted to fit current story needs
- Scalable threat/reward based on party level
- Multiple entry points for different play styles
- Connections to overarching campaign themes

## Plot Hooks

- Strange accidents suggest ancient magic
- A noble needs help stealing before dawn
- A map reveals a betrayal about this place

## Treasure Parcels

### Parcel A (Combat Victory)
- `dice: `dice: 2d6`` × 10 gp
- Potion of Healing
- Clue to next location

### Parcel B (Exploration)
- Magic item (roll on Table F)
- Ancient map fragment
- `dice: `dice: 1d4`` gems (50 gp each)

## Timeline
- Key events
- Deadlines

## NPCs Involved
- Quest giver
- Antagonists

## Complications
- Potential problems
- Twists

## Alternative Solutions
- Non-combat options
- Creative approaches
