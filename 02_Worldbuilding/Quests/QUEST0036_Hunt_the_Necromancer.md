---
aliases: ["Hunt the Necromancer", "hunt the necromancer", "Hunt Necromancer", "Necromancer the Hunt"]
tags: [generated]
id: QUEST0036
created: 2025-08-15T04:09:51.384Z
generator: mega_generator_10k
---
# Hunt the Necromancer

*Negotiate Quest for levels 2-6*

**ID**: QUEST0036

## Quest Giver
**Leopold Goldleaf** - Desperate noble
- **Location**: Merchant guild hall
- **Motivation**: Following divine mandate

## Objectives
### Primary
Broker peace between the merchant guild and the thieves guild

### Secondary
- Sabotage enemy resources

## Background
Years ago, a series of events set this quest in motion. undefined. Years ago, they witnessed something they shouldn't have, which changed their life forever. Now, circumstances demand immediate action.

## Key Locations
- **Marshstead** (City): Key location for quest progression
- **NewGate** (Dungeon): Key location for quest progression
- **Fallsstead** (Wilderness): Key location for quest progression
- **Grovestead** (Temple): Key location for quest progression

## Opposition
### Enemies
- **Corrupt Guards** led by Leopold Brightblade
  - Strength: 12 combatants
  - Tactics: Aggressive but cunning

### Obstacles
- Moral dilemma
- Moral dilemma

## Complications
- Rival adventuring party competing

## Plot Twist
*Reveal at appropriate moment:*
- The goal was a lie all along

## Moral Choices
### Enemy offers valuable information for mercy
**Options**:
1. Show mercy and gain intel - Future ally but justice denied
2. Execute for crimes committed - Justice served but opportunity lost

## Rewards
### On Success
- **Gold**: 1,990 gp
- **Experience**: 2,000 XP
- **Magic Items**: Eldritch Tome of Power
- **Other**: Map to treasure

## Consequences
- **Success**: Region becomes safer
- **Failure**: Innocent people die

## DM Notes
- Adjust difficulty based on party composition
- The hidden objective adds complexity for experienced players
- Moral choices should have lasting campaign impacts
- Consider how this quest ties into larger campaign arcs

---
*Generated: 2025-08-15T03:20:13.158Z*
