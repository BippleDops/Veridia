---
created: '2025-08-11'
status: complete
tags:
- access/dm-only
- both
- campaign/session
- complete
- gm_resource
- horror
- lore
- status/complete
- world/aquabyssos
- world/both
title: Encounter Sheet — D-005 Shadow Bloom
type: Lore
updated: '2025-08-13T12:34:03.198897+00:00'
world: Both
---



# D-005 Shadow Bloom

## Read-Aloud
> The darkness thickens until it has weight. Your lamps shrink into trembling halos, and every movement trails a second self in black.

## Setup
- Zone: Abyss (DC 18) • Category: Horror
- Trigger: Shadow density spike near trench wall
- Time Pressure: 10 minutes until peak

## Mechanics
- Group Sanity DC 15 on first exposure, then every 5 minutes
- Sensor Scan DC 16 distinguishes true from echo returns
- Ship Sanity (if living) check DC 14 on peak

## Outcomes
- Success: Crew resists; next Sanity save advantage for 24h
- Partial: 1 crew short-term madness; Helm Disadvantage on next Maneuver
- Fail: 1 long-term madness; ship Sanity -2 if living

## Stat Blocks
### Shadow Bloom (Environmental Hazard)
AC —; HP —; Area 300 ft radius; Saves: Sanity DC 15 (group), Sensor DC 16
Effects: Dim bright light to dim; creates echo images; imposes Disadvantage on Perception relying on sight
Countermeasures: Anchor hymn (Religion DC 14) reduces DCs by 1 for duration

## Integrations
- [[Sanity System]]: apply madness tables
- [[Complete Reality Merger System]]: on flicker, roll Merger Complications (severity -1 if anchor active)
- [[Memory Trading Economy System|Memory Economy]]: harvestable shadow fragments (illegal, 150 gp)

## Scaling
- Add Reality check in merger flicker (Integrity -1 on fail)
- In Hadal, Sanity DC +2 and hazard lingers

## Variants
- Whisper offers shortcut for a memory payment
- Predatory shadow shapes hunt isolated crew

## Quick-Run
- Call group Sanity DC 15; on fail, roll madness
- Sensor DC 16 to clear a path; else Disadvantage on evasion
- Optional anchor hymn to reduce DCs

## Player-Facing Summary

D 005 Shadow Bloom is a undersea element of the setting, known for bioluminescent glow and pressure-glass. Its presence anchors ongoing storylines and offers clear player choices.

## Lore Details

Legends speak of D 005 Shadow Bloom as a nexus where past and present converge. Locals describe subtle omens—shifts in currents, a dimming of lanternfish, or whispers on the wind—that herald change around D 005 Shadow Bloom.

## Adventure Hooks

- A rumor ties D 005 Shadow Bloom to a missing shipment, linking factions with competing claims.
- An NPC seeks discreet help at D 005 Shadow Bloom to avert a public scandal.
- A map overlay reveals a hidden approach to D 005 Shadow Bloom active only during specific tides/storms.

## DM Notes

Play up tactile detail: sounds, pressure/wind changes, and meaningful symbology. Offer two clear approaches (stealth vs. parley) and one wildcard complication tied to a faction clock. Reward scouting and map use.

<!-- enriched: true -->
