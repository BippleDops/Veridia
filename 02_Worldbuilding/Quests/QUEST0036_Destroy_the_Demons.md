---
aliases: ["Destroy the Demons", "destroy the demons", "Destroy Demons", "Demons the Destroy"]
tags: [generated]
id: QUEST0036
created: 2025-08-15T04:09:51.384Z
generator: mega_generator_10k
---
# Destroy the Demons

*Negotiate Quest for levels 2-6*

**ID**: QUEST0036

## Quest Giver
**Leopold Goldleaf** - Religious leader
- **Location**: Merchant guild hall
- **Motivation**: Driven by greed

## Objectives
### Primary
Broker peace between the merchant guild and the thieves guild

### Secondary
- Document findings for scholars
- Minimize civilian casualties

## Background
Years ago, a series of events set this quest in motion. undefined. Years ago, they lost everything in a fire, which changed their life forever. Now, circumstances demand immediate action.

## Key Locations
- **OldGate** (City): Key location for quest progression
- **Peakstead** (Dungeon): Key location for quest progression

## Opposition
### Enemies
- **Corrupt Guards** led by Leopold Brightblade
  - Strength: 12 combatants
  - Tactics: Aggressive but cunning
- **Environmental**: Natural hazards and traps
  - Pit traps
  - Poison gas
  - Collapsing ceilings

### Obstacles
- Moral dilemma
- Time-sensitive element

## Complications
- Unexpected magical phenomena
- Disease outbreak

## Plot Twist
*Reveal at appropriate moment:*
- The quest giver is the true villain

## Moral Choices
### Enemy offers valuable information for mercy
**Options**:
1. Show mercy and gain intel - Future ally but justice denied
2. Execute for crimes committed - Justice served but opportunity lost

## Rewards
### On Success
- **Gold**: 1,385 gp
- **Experience**: 2,000 XP
- **Magic Items**: Eldritch Tome of Power
- **Other**: Noble title

## Consequences
- **Success**: New trade routes open
- **Failure**: Darkness consumes the land

## DM Notes
- Adjust difficulty based on party composition
- The hidden objective adds complexity for experienced players
- Moral choices should have lasting campaign impacts
- Consider how this quest ties into larger campaign arcs

---
*Generated: 2025-08-15T03:21:04.088Z*
