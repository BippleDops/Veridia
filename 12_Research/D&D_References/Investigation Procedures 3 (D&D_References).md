---
title: Investigation Procedures 3 (D&D_References)
type: note
tags:
- note
created: '2025-01-15'
modified: '2025-01-15'
---

# Investigation Procedures 3

---
title: Investigation Procedures 3
type: Lore
tags:
- lore
- both
- status/complete
- mechanics/exploration
- worldbuilding
- content/lore
- world/both
- research
- procedures
- active
- complete
- world/aquabyssos
created: '2025-08-11'
modified: '2025-08-14'
status: complete
date_created: '2025-08-11'
date_modified: '2025-08-11'
updated: '2025-08-13T12:34:28.597486+00:00'
world: Both
---

# Investigation Procedures

*"Truth flows like the deep currents - follow the proper channels, and even the darkest secrets surface." - Spymaster Crow*

## 🔧 Deep Evaluation Improvements

*20 targeted improvements identified*

### Connection Improvements

- Add cross-references to related notes

### Enhancement Improvements

- Add 12_Research-specific enhancement

## Overview
Investigation procedures in Aquabyssos follow a complex web of formal protocols, shadow networks, and ancient traditions adapted to underwater society. The multi-layered nature of the city creates unique challenges for gathering information, requiring specialized techniques for different depth levels and social strata. These procedures have evolved to account for the three-dimensional nature of underwater cities, the presence of multiple intelligent species, and the constant threat of pressure-related complications.

The formal investigation system operates through the Pearl Guard's Detective Division, while informal networks span from the Shadow Warren's intelligence operations to merchant guild information brokers. Each system has developed its own methods for handling evidence preservation in aquatic environments, witness protection across vast vertical territories, and the unique challenges of pursuing leads through both inhabited districts and the dangerous depths beyond city limits.

### Origins
The earliest investigation procedures in Aquabyssos emerged from ancient Merfolk traditions of truth-seeking through echolocation-based interrogation techniques. These methods relied on the ability to detect physiological changes in subjects through sound waves, creating a natural form of truth detection that predated magical means. As the city grew to accommodate multiple species, these techniques were adapted and formalized into protocols that could work across different physiologies.

The integration of Surface dwellers brought new concepts of evidence collection and witness testimony, while the unique challenges of underwater crime scenes required innovative preservation methods. Early investigators developed specialized containers using magical preservation spells and pressure-resistant materials to maintain evidence integrity across different depth zones.

### Development
During the Great Unification period, investigation procedures became standardized across all districts of Aquabyssos. The Pearl Guard established the Detective Division, creating formal ranks and procedures that combined the best practices from all constituent species. This period saw the development of the evidence preservation protocols still used today, including the use of crystal matrices to store sensory memories and pressure-sealed evidence chambers.

The Shadow Warren's influence during this time led to the parallel development of covert investigation techniques, creating a shadow system that operates alongside official procedures. These methods focus on information gathering through social networks, economic pressure, and the exploitation of the city's complex political relationships.

### Current Era
Modern investigation procedures in Aquabyssos represent a sophisticated blend of traditional techniques, magical enhancement, and adaptation to contemporary threats. The rise of the Silverscale Conspiracy and increasing Surface tensions have driven innovations in counter-intelligence and deep-cover operations. Current procedures must account for threats ranging from political assassination to reality-altering magical phenomena.

The introduction of crystal corruption as a investigative complication has led to specialized protocols for handling contaminated evidence and witnesses. Investigation teams now require regular screening for corruption exposure, and specialized containment procedures for corruption-related crimes.

### In Aquabyssos
Investigation procedures reflect the city's values of community protection and collective responsibility. The concept of "current-truth" - information that flows naturally through social networks - is considered as valuable as formal evidence. Citizens are expected to cooperate with investigations as part of their civic duty, though the multi-layered society creates complex loyalties that investigators must navigate carefully.

The vertical nature of the city has created distinct investigation cultures at different depth levels. Upper district investigations tend to be more formal and politically sensitive, while deeper investigations may rely more heavily on traditional methods and community knowledge.

### In Aethermoor
Surface dwellers initially found Aquabyssos investigation procedures overly complex and intrusive, particularly the use of magical truth detection and the expectation of community involvement. However, as relations have improved, many Surface investigators have adopted underwater evidence preservation techniques for their own challenging environments.

The exchange of investigation techniques has been one of the most successful areas of inter-world cooperation, with joint training programs producing investigators capable of working in both environments.

### The Tidecrest Incident
A breakthrough case where proper procedure allowed investigators to solve a murder that occurred across three different depth zones, establishing precedent for complex multi-environment crimes.

### Shadow Warren Infiltration
The discovery that investigation procedures had been compromised by Shadow Warren operatives led to a complete overhaul of security protocols and the development of counter-intelligence measures.

## Current Developments
The recent increase in crystal corruption cases has prompted development of specialized investigation teams with enhanced protective equipment. New procedures are being developed for investigating reality-altering crimes where traditional evidence may be unreliable or dangerous to examine.

The growing cooperation with Surface authorities has led to joint investigation protocols for crimes that span both worlds, requiring new diplomatic and procedural frameworks.

## Adventure Hooks
1. A murder investigation leads to evidence that someone has been systematically corrupting investigation procedures
2. An ancient investigation technique reveals that a current case is connected to crimes from centuries ago
3. The party must investigate a crime scene that phases between realities, requiring new procedures
4. Evidence from a formal investigation contradicts information from shadow networks, suggesting conspiracy

### Early Campaign
Introduce through minor crime investigations that demonstrate the unique challenges of underwater detective work and the importance of following proper procedures.

### Mid Campaign
Procedural knowledge becomes crucial for uncovering larger conspiracies and navigating the complex political implications of major investigations.

### Late Campaign
Mastery of investigation procedures allows the party to uncover the deepest secrets of Aquabyssos and solve mysteries that have persisted for generations.

### Running This Element
Use investigation procedures to create realistic consequences for player actions and to demonstrate the sophisticated nature of Aquabyssos society. Allow players to learn and benefit from following proper procedures while showing the limitations of formal systems.

### Common Questions
**Q:** How do investigation procedures handle non-humanoid suspects?
**A:** Specialized protocols exist for each intelligent species, with cultural liaisons and species-appropriate interrogation techniques.

**Q:** What happens when investigations cross into the deep dangers beyond the city?
**A:** Special deep-current investigation teams with enhanced equipment and emergency protocols handle such cases.

### Customization Options
Adapt procedures to reflect your campaign's tone - more bureaucratic for political intrigue, more action-oriented for adventure-focused games, or more mysterious for investigation-heavy scenarios.

## Player-Facing Summary

Investigation Procedures is a undersea element of the setting, known for bioluminescent glow and pressure-glass. Its presence anchors ongoing storylines and offers clear player choices.

## Lore Details

Legends speak of Investigation Procedures as a nexus where past and present converge. Locals describe subtle omens—shifts in currents, a dimming of lanternfish, or whispers on the wind—that herald change around Investigation Procedures.

## DM Notes

Play up tactile detail: sounds, pressure/wind changes, and meaningful symbology. Offer two clear approaches (stealth vs. parley) and one wildcard complication tied to a faction clock. Reward scouting and map use.

## Prophecy Connection

Mentioned in The First Prophecy of Shadows

## Plot Hooks

- A stranger needs help finding before winter
- A prisoner has gone missing and chaos spreads

## 12_Research Specific Content

Contextual improvement based on 12_Research

## 12_Research Specific Content

Contextual improvement based on 12_Research
