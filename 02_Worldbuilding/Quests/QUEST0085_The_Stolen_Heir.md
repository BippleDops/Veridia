---
aliases: ["The Stolen Heir", "the stolen heir", "Heir Stolen The"]
tags: [generated]
id: QUEST0085
created: 2025-08-15T04:09:51.412Z
generator: mega_generator_10k
---
# The Stolen Heir

*Explore Quest for levels 5-9*

**ID**: QUEST0085

## Quest Giver
**Fiora Brightblade** - Religious leader
- **Location**: Noble quarter
- **Motivation**: Driven by greed

## Objectives
### Primary
Map the uncharted the sunken temple and report findings

### Secondary
- Maintain cover identity

### Hidden Objective
*DM Only: Employer has ulterior motives*

## Background
Years ago, a series of events set this quest in motion. undefined. Years ago, they lost everything in a fire, which changed their life forever. Now, circumstances demand immediate action.

## Key Locations
- **LowerFalls** (City): Key location for quest progression
- **OldFalls** (Dungeon): Key location for quest progression
- **Peakwick** (Wilderness): Key location for quest progression
- **Fallswick** (Temple): Key location for quest progression

## Opposition
### Enemies
- **Wild Beasts** led by Fiora Fireborn
  - Strength: 28 combatants
  - Tactics: Aggressive but cunning

### Obstacles
- Conflicting information
- Conflicting information

## Complications
- Disease outbreak

## Plot Twist
*Reveal at appropriate moment:*
- Enemies become necessary allies

## Moral Choices
### Civilian lives vs mission success
**Options**:
1. Save civilians and risk failure - Heroes to the people but objective compromised
2. Complete mission at any cost - Success but blood on hands

## Rewards
### On Success
- **Gold**: 4,884 gp
- **Experience**: 5,000 XP

- **Other**: Property deed

## Consequences
- **Success**: Ancient evil remains sealed
- **Failure**: Evil spreads unchecked

## DM Notes
- Adjust difficulty based on party composition
- The hidden objective adds complexity for experienced players
- Moral choices should have lasting campaign impacts
- Consider how this quest ties into larger campaign arcs

---
*Generated: 2025-08-15T03:21:04.093Z*
