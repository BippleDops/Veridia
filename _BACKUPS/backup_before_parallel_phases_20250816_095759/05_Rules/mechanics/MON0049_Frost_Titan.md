---
aliases: ["Frost Titan", "frost titan", "Titan Frost"]
tags: [generated]
id: MON0049_Frost_Titan
created: "2025-08-15T04:09:51.434Z"
generator: mega_generator_10k
---
# Frost Titan

*Tiny Beast, CR 10*

- **HP**: 163
- **AC**: 16
- **Speed**: 30 ft.

## Abilities
- **Darkvision 60 ft.**

## Actions
- **Claw: +7 to hit, `dice: 2d6`+4 slashing damage**
- **Bite: +7 to hit, `dice: 2d8`+4 piercing damage**
- **Breath Weapon (Recharge 5-6): 30 ft. cone, `dice: 6d6` damage**
- **Frightful Presence: Wisdom save **DC 15** or frightened**
- **Teleport: Teleports up to 60 ft.**
- **Summon Minions: Calls `dice: 1d4` lesser creatures**

## Description
A being of pure elemental energy given form

---
*Generated: 2025-08-15T03:21:47.466Z*

## Habitat
- Natural environment and territories

## Behavior
- Typical actions and patterns

## Ecology
- **Diet**: What they eat
- **Predators**: What hunts them
- **Prey**: What they hunt

## Life Cycle
- **Young**: Juvenile form
- **Adult**: Mature form
- **Lifespan**: How long they live

## Social Structure
- **Pack Behavior**: Group dynamics
- **Territory**: Range and claims

## Weaknesses
- **Vulnerabilities**: Susceptibilities
- **Fears**: What scares them

## Tracks & Signs
- **Footprints**: Track appearance
- **Other Signs**: Evidence of presence

## Uses
- **Parts Value**: Useful components
- **Domestication**: Taming possibility

## Variations
- **Subspecies**: Different types
- **Regional Differences**: Local variants

## Encounter Behavior
- **Initial Reaction**: First response
- **Combat Preference**: Fighting style

## Lore
- **Myths**: Stories about them
- **Truth**: Actual facts