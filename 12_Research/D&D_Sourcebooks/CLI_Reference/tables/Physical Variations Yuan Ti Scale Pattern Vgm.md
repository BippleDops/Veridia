# Physical Variations Yuan Ti Scale Pattern Vgm

---
title: Physical Variations Yuan Ti Scale Pattern Vgm
aliases:
- Physical Variations; Yuan-ti Scale Pattern
type: note
tags:
- both
- research
- world/both
- active
- note
- status/in-progress
- ttrpg-cli/compendium/src/5e/vgm
created: 2025-07-23
modified: '2025-08-14'
status: active
cssclasses: json5e-note
obsidianUIMode: preview
updated: '2025-08-13T12:34:03.541097+00:00'
world: Both
---



# Physical Variations; Yuan-ti Scale Pattern
*Source: Volo's Guide to Monsters p. 97* 

**Yuan-ti Scale Pattern**

`dice: [](physical-variations-yuan-ti-scale-pattern-vgm.md#^yuan-ti-scale-pattern)`

| dice: d20 | Scale Pattern |
|-----------|---------------|
| 1-5 | Mottled |
| 6-7 | Random |
| 8-10 | Reticulated |
| 11-15 | Speckled |
| 16-20 | Striped |
^yuan-ti-scale-pattern

## Player-Facing Summary

Physical variations yuan ti scale pattern vgm is a undersea element of the setting, known for bioluminescent glow and pressure-glass. Its presence anchors ongoing storylines and offers clear player choices.

## Lore Details

Legends speak of physical variations yuan ti scale pattern vgm as a nexus where past and present converge. Locals describe subtle omens—shifts in currents, a dimming of lanternfish, or whispers on the wind—that herald change around physical variations yuan ti scale pattern vgm.

## Adventure Hooks

- A rumor ties physical variations yuan ti scale pattern vgm to a missing shipment, linking factions with competing claims.
- An NPC seeks discreet help at physical variations yuan ti scale pattern vgm to avert a public scandal.
- A map overlay reveals a hidden approach to physical variations yuan ti scale pattern vgm active only during specific tides/storms.

## DM Notes

Play up tactile detail: sounds, pressure/wind changes, and meaningful symbology. Offer two clear approaches (stealth vs. parley) and one wildcard complication tied to a faction clock. Reward scouting and map use.

<!-- enriched: true -->


## Related

*Links to related content will be added here.*
